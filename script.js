...
const switchPlayer = function () {
  ...
  if (player1.active) {
    ...
    player1.active = false;
    player2.active = true;
    roundArea.textContent = `Round ${player2.round} Changing player...\n ${player2.name}, now it's your turn, please roll dice`;
  } else {
    cart2.classList.remove("border-dark");
    cart1.classList.add("border-dark");
    player2.active = false;
    player1.active = true;
    roundArea.textContent = `Round ${player1.round} Changing player...\n ${player1.name}, now it's your turn, please roll dice`;
  }
};
const displayDice = function (dice) {
  document.querySelector(".dice").textContent = [...dice];
...
btnStart.addEventListener("click", function (e) {
  ...
  total1.textContent = 0;
  total2.textContent = 0;
  diceArea.textContent = `${player1.name}
   please Roll Dice`;
});
...
btnRoll.addEventListener("click", function () {
  ...
  diceArea.textContent = dice = rollDice();
  roundArea.textContent = `Round ${activePlayer.round}`;
  //calculate current score
  current = currentScore(activePlayer.round, dice);
  ...
  if (current > 0) {
    ...
    // activePlayer.scores.push(total);
    // console.log(activePlayer.name, activePlayer.scores);
  } else {
    if (total > 6 && player1.round === player2.round) {
      if (player1.active) {
        player1.round++;
        player1.roundScore.push(Number(total1.textContent));
        total1.textContent = 0;
      } else {
        player1.round++;
        player2.round++;
        player1.roundScore.push(Number(total1.textContent));
        player2.roundScore.push(Number(total2.textContent));
        total1.textContent = 0;
        total2.textContent = 0;
      }
    } else if (player1.round !== player2.round) {
      player2.round++;
      player2.roundScore.push(Number(total2.textContent));
      total2.textContent = 0;
    }
